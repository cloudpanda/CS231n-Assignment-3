...
class CaptioningRNN(object):
  ...
  def loss(self, features, captions):
    ...
    captions_in_vectors, word_embed_cache = word_embedding_forward(captions_in, W_embed)
    if self.cell_type == 'rnn':
        hidden, rnn_cache = rnn_forward(captions_in_vectors, initial_hidden_state, Wx, Wh, b)
    ...
    if self.cell_type == 'rnn':
       ...
       dcaptions_in_vectors, dinitial_hidden_state, grads['Wx'], grads['Wh'], grads['b'] = rnn_backward(dhidden, rnn_cache)
       
       grads['W_embed']= word_embedding_backward(dcaptions_in_vectors, word_embed_cache)
       grads['W_proj'] = np.dot(features.T, dinitial_hidden_state)
  ...
  def sample(self, features, max_length=30):
    ...
    prev_h = initial_hidden_state  # Previous hidden state
    prev_c = np.zeros_like(initial_hidden_state)  # Previous cell state
    ...
    for t in xrange(max_length):  # Let's go over the sequence
        ...
        # Squeeze unnecessary dimension and get the best word idx
