...
def rnn_backward(dh, cache):
  ...
  for i in reversed(xrange(T)):
    ...
    dx_i, dprev_h_i, dWx_i, dWh_i, db_i= rnn_step_backward(dh_current, cache[i])
    dx+= dx_i
    db+= db_i
    dWh+= dWh_i
    dWx+= dWx_i
...
def word_embedding_backward(dout, cache):
  ...
  #Add the gradient values to dW from dout at the indices given by x
  np.add.at(dW, x, dout)
